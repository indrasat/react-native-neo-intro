...
export function intro(opts = {}) {
    ...
    function stop() {
        clearTimeout(stepTimer);
        clearTimeout(timer);
        sibling != null && sibling.destroy();
        sibling = null;
        index = 0;
        try {
            AsyncStorage.setItem('INTRO', JSON.stringify(false));
        } catch (error) {
            // Error saving data
            console.debug("[Async Storage] Set item error")
        }
    }
    ...
    function toStep(index) {
        ...
        .then((res) => {
            ...
            timer = setTimeout(() => {
                ...
                refModal.content = typeof content === 'string' ? <View><Text style={{color:'#fff'}}>{content}</Text></View> : content;
                refModal.forceUpdate(() => {
    ...
    render() {
        ...
        return (
            <View style={[styles.container, {zIndex}]}>
                <TouchableOpacity activeOpacity={0.8} style={[styles.sibling]}/>
